...
- **Add levels as data.** New levels go in `js/levels.js` — no engine changes
  needed. Follow the schema documented at the top of that file.
...
- **Don't break the leaderboard contract.** `js/leaderboard.js` is shared with
  the other games; keep its public API (`top`, `submit`, `qualifies`, `mode`)
  stable and keep the localStorage fallback working when Supabase isn't configured.
...
suggest a feature (a new level, enemy or power-up). Please include steps to
